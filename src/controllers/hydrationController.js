...
const getSummary = async (req, res) => {
  ...
  const [logs, user] = await Promise.all([
    ...
    User.findById(userId).select('hydrationGoalOz')
  ]);
...
const deleteLog = async (req, res) => {
  const { id } = req.params;
  const result = await HydrationLog.deleteOne({ _id: id, userId: req.user._id });
