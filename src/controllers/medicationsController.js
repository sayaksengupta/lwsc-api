...
const createSchedule = async (req, res) => {
  const schedule = await MedicationSchedule.create({ ...req.body, userId: req.user._id });
...
const deleteSchedule = async (req, res) => {
  const { id } = req.params;
  const result = await MedicationSchedule.deleteOne({ _id: id, userId: req.user._id });
...
const listIntakes = async (req, res) => {
  ...
  const intakes = await MedicationIntake.find(filter).sort({ dateTime: -1 }).lean();
  res.json(intakes);
};
  ...
  const history = schedules.map(s => {
    ...
    while (current <= end) {
      ...
      if (s.daysOfWeek.includes(dayOfWeek)) {
        days.push({
          date: dayStr,
          value: intakes.some(i => i.scheduleId.toString() === s._id.toString() && i.dateTime.toISOString().split('T')[0] === dayStr) ? 1 : 0
        });
      }
...
const adherence = async (req, res) => {
  ...
  while (current <= end) {
    ...
    const expected = await MedicationIntake.countDocuments({
      userId: req.user._id,
      dateTime: { $gte: new Date(dayStr), $lt: new Date(current.getTime() + 86400000) },
      status: 'Taken'
    });
    ...
    days.push({ date: dayStr, value: total > 0 ? (expected / total >= 0.8 ? 1 : 0) : 0 });
    current.setDate(current.getDate() + 1);
  }
