...
const comparison = async (req, res) => {
  ...
  // Fetch all data for ACTIVE profile
  ...
  if (metricList.includes("pain")) {
    result.painByDay = Object.entries(days).map(([date, data]) => ({
      ...
      avgIntensity:
        data.pain.count > 0
          ? Number((data.pain.sum / data.pain.count).toFixed(2))
          : 0,
    }));
  }
  ...
  if (metricList.includes("mood")) {
    result.moodByDay = Object.entries(days).map(([date, data]) => ({
      ...
      avgIntensity:
        data.mood.count > 0
          ? Number((data.mood.sum / data.mood.count).toFixed(2))
          : 0,
    }));
  }
...
const correlations = async (req, res) => {
  ...
  const correlation =
    denominator === 0 ? 0 : Number((numerator / denominator).toFixed(3));
