...
const getWidgets = async (req, res) => {
  ...
  const [
    ...
    HydrationLog.find({ userId, date: { $gte: startOfDay, $lt: endOfDay } }).lean(),
    MedicationIntake.find({ userId, dateTime: { $gte: startOfDay, $lt: endOfDay } }).lean(),
  ...
  // 1. Pain Summary
  const painSummary = painToday.length > 0
    ? {
        count: painToday.length,
        avgIntensity: Number((painToday.reduce((s, p) => s + p.intensity, 0) / painToday.length).toFixed(1)),
        highest: Math.max(...painToday.map(p => p.intensity))
      }
    : { count: 0, avgIntensity: 0, highest: 0 };

  // 2. Mood Summary
  const moodSummary = moodToday.length > 0
    ? {
        avgIntensity: Number((moodToday.reduce((s, m) => s + m.intensity, 0) / moodToday.length).toFixed(1)),
  ...
  const hydrationProgress = {
    ...
    percentage: Math.min(100, Number(((totalOz / (user.hydrationGoalOz || 64)) * 100).toFixed(0)))
  };
  ...
  const adherence = expectedDoses > 0
    ? { taken: takenDoses, expected: expectedDoses, percentage: Math.round((takenDoses / expectedDoses) * 100) }
